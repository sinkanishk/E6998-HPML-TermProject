...
def _compress_model(r_model_dict :dict, r_compress_ratio :float, n_layers : int, d_visual: bool, d_text : bool):
    """
...
#         value = r_model_dict[key]
    ...
    for key,value in r_model_dict.items():
        skip_layer = True
        if key.startswith('visual') and idx%7==0 and idx-last_idx_vision>14:
            skip_layer = False
            last_idx_vision = idx
            print(f"Visual - {idx}")
        
        if key.startswith('transformer') and idx%5==0 and idx-last_idx_transformer>10:
            print(f"Transformer - {idx}")
            last_idx_transformer = idx
            skip_layer = False
